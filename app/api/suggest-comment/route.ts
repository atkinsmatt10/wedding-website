...
export async function POST(req: Request) {
  try {
    ...
    if (isAttending === 'yes') {
      ...
      basePrompt = `Write a short, positive comment expressing excitement about attending Nicole & Matt's wedding. ${songContext} Sign it off with the name: ${nameToUse}`;
    } else if (isAttending === 'no') {
      // Keep polite decline, remove explicit RSVP thanks if possible
      basePrompt = `Write a short, polite, and regretful comment about being unable to attend Nicole & Matt's wedding. ${songContext} Wish them a wonderful celebration. Sign it off with the name: ${nameToUse}`;
    } else {
       // Focus on politeness and song, remove explicit RSVP thanks
       basePrompt = `Write a short, polite comment regarding Nicole & Matt's wedding RSVP. ${songContext} Sign it off with the name: ${nameToUse}`;
    }
    ...
    if (existingComment && existingComment.trim() !== '') {
      ...
      finalPrompt = `The user has started writing the following comment regarding Nicole & Matt's wedding RSVP: "${existingComment}". Continue writing based on the user's start, keeping the tone appropriate for the situation (${isAttending === 'no' ? 'regretful decline' : 'happy acceptance'}). Focus on the event/song excitement (if applicable) rather than the RSVP action itself. ${songContext} Make it sound natural and complete the thought. Final response should be just the suggested continuation/completion, signed off with the name: ${nameToUse}. Do not repeat the user's starting text in your response.`;
    }
